...
def img2mseCLEARED(x, y, c):
    return torch.sum(torch.square(x - y)*c)/(torch.sum(c)+1e-5)
...
def volumetric_rendering(rgb, density, t_vals, dirs, white_bkgd):
    ...
    return comp_rgb, acc, weights, alpha, dists

def volumetric_rendering_composed(rgb, rgb_fog, density, density_fog, t_vals, dirs, white_bkgd):
    ...
    dists = dists * torch.norm(dirs[..., None, :], dim=-1)
    alpha = 1.0 - torch.exp(-density[..., 0] * dists)
    alpha_fog = 1.0 - torch.exp(-density_fog[..., 0] * dists)
    alpha_composed = 1.0 - torch.exp(-(density+density_fog)[..., 0] * dists)
    accum_prod = torch.cat(
        [
            torch.ones_like(alpha_composed[..., :1]),
            torch.cumprod(1.0 - alpha_composed[..., :-1] + eps, dim=-1),
        ],
        dim=-1,
    )

    weights = alpha * accum_prod
    weights_fog = alpha_fog * accum_prod

    comp_rgb = (weights[..., None] * rgb).sum(dim=-2)
    comp_rgb_fog = (weights_fog[..., None] * rgb_fog).sum(dim=-2)
    comp_rgb += comp_rgb_fog

    depth = (weights * t_vals).sum(dim=-1)
    acc = weights.sum(dim=-1)
    inv_eps = 1 / eps

    if white_bkgd and False:
        comp_rgb = comp_rgb + (1.0 - acc[..., None])

    return comp_rgb, acc, weights, alpha_composed, dists

def get_alpha(density, t_vals, dirs):
