...
class LitModel(pl.LightningModule):
    ...
    def alter_gather_cat(self, outputs, key, image_sizes, channels_number = 3):
        each = torch.cat([output[key] for output in outputs])
        if len(each.shape) == 1 :
            each = torch.stack([each]*3, dim = -1 )
            print(each.shape)
    ...
    def alter_gather_cat_raw(self, outputs, key, image_sizes):
        each = torch.cat([output[key] for output in outputs])
        print(each.shape, "each.shape,")
        if len(each.shape) == 2 : 
            each = each[...,None]
        assert( len(each.shape) == 3 )
        n_points = each.shape[1]
        ...
        for (h, w) in image_sizes:
            try : 
                ret.append(all[curr : curr + h * w].reshape(h, w, n_points, -1))
                curr += h * w
            except Exception as e :
                print("error:", e )
        return ret
