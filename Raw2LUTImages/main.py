...
def parsArgs():
    ...
    parser.add_argument('--daytime', '-t', help='Set day/night time. True indicates daytime, False nighttime. Helpful when meta label does not exist', default=None)
...
if __name__ == '__main__':
    ...
    for sample in os.listdir(os.path.join(args.root, args.image_folder)):
        image_lut = RI.process_lut(os.path.join(args.root, args.image_folder, sample),
                                   meta_path=os.path.join(args.root, args.meta_folder, sample.replace('.tiff', '.json')),
