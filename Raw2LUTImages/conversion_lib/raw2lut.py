...
from Raw2LUTImages.conversion_lib.basic_utils import read_image_intrinsic, read_tiff_image, save_tiff_image, check_image, read_meta_file, parse_day_night, apply_clahe_8bit
import Raw2LUTImages.conversion_lib.decompand as decompand
import numpy as np
import cv2
import os
...
class Convert_image():
    ...
    def process_lut(self, image_path, clahe=False, meta_path=None):
        """
        ...
        image_bit = apply_clahe_8bit(image_bit) if clahe else image_bit
    ...
    def process_lut_16_bit(self, image_path, clahe=False, meta_path=None):
        """
        Takes a raw data image and converts it to a 8 bit RGB image with for visual inspection.
        Images are applied with an hand crafted image enhancement process including a gamma correction and
        contrast enhancement. This approach reimplements the process for the images in cam_stereo_left_lut.
        ...
        # image_bit = np.right_shift(image_bayer, 8).astype(np.uint8)
        image_bit = apply_clahe_8bit(image_bayer) if clahe else image_bayer
        self.PC.rectifyImage(image_bit, image_bit)
        return image_bit
    ...
    def left_to_right_correction(self,image,color_matrix_path):
        '''

        :param image: bgr image
        :param color_matrix_path: contains weight and bias for conversion from left to right, which applies to bgr image
        :return:
        '''
        image = image.astype(np.float32) / 255.
        right_image_reshaped = image.reshape((image.shape[0] * image.shape[1], image.shape[2]))

        weights = np.load(os.path.join(color_matrix_path,'weights.npy'))
        bias = np.load(os.path.join(color_matrix_path,'bias.npy'))
        right_image_corrected = (np.linalg.inv(weights) @ (right_image_reshaped - bias).T).T
        right_image_corrected = right_image_corrected.reshape(image.shape)

        return np.clip(right_image_corrected * 255., 0., 255.).astype(np.uint8)

    def process_rect_raw_to_lut(self,image_raw=None, image_path=None,color_correction=True,color_matrix_path = None):
        """
        Takes a rectified and bayered 12 bits raw image and converts it to a 8-bits lut image,
        by default, take left image and convert it the right color space
        """
        if image_raw is None:
            image_raw = read_tiff_image(image_path)
        image = self.decompand_lut[image_raw]
        image_lut = self.daytime_lut[image]
        image_bit = np.right_shift(image_lut, 8).astype(np.uint8)

        if color_correction:
            image_bit = self.left_to_right_correction(image_bit,color_matrix_path)
